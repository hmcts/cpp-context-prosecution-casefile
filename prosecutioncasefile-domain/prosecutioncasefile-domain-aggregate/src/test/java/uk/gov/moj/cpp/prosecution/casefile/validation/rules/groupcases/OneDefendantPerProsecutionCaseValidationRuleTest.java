...
public class OneDefendantPerProsecutionCaseValidationRuleTest {

    @Mock
    private ReferenceDataQueryService referenceDataQueryService;

    @InjectMocks
    private OneDefendantPerProsecutionCaseValidationRule oneDefendantPerProsecutionCaseValidationRule;

    @Test
    public void shouldNotReturnAnyValidationErrorsWhenOneDefendantAvailablePerProsecutionCase() {
        ...
        final ValidationResult problem = oneDefendantPerProsecutionCaseValidationRule.validate(groupProsecutionList, referenceDataQueryService);
    ...
    @Test
    public void shouldReturnValidationErrorsWhenMoreThanOneDefendantAvailablePerProsecutionCase() {
        ...
        final ValidationResult result = oneDefendantPerProsecutionCaseValidationRule.validate(groupProsecutionList, referenceDataQueryService);

        Problem problem = result.problems().stream().findFirst().orElse(null);

        assertThat(problem.getCode(), is("MORE_THAN_ONE_DEFENDANT_PER_PROSECUTION_CASE"));
        assertThat(problem.getValues().size(), is(1));
        assertThat(problem.getValues().get(0).getKey(), is("more.than.one.defendant.per.prosecution.case"));
        assertThat(problem.getValues().get(0).getValue(), is(prosecutionCaseId2.toString()));
