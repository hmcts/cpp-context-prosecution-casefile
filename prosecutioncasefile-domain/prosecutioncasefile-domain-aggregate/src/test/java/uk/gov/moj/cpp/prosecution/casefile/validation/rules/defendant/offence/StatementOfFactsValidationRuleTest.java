...
public class StatementOfFactsValidationRuleTest {
    ...
    @Test
    public void shouldReturnEmptyListWhenStatementOfFactsIsPresentForSummonsCaseWithSummonsCodeM() {
        ...
        Optional<Problem> optionalProblem = new StatementOfFactsValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
    ...
    @Test
    public void shouldReturnProblemWhenStatementOfFactsIsNotPresentForSummonsCaseWithSummonsCaseM() {
        when(defendantWithReferenceData.getDefendant().getOffences()).thenReturn(buildMockOffencesWithoutStatementOfFacts());
        when(defendantWithReferenceData.getCaseDetails().getSummonsCode()).thenReturn("M");

        final Optional<Problem> optionalProblem = new StatementOfFactsValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
                .problems().stream().findFirst();
    ...
    private List<Offence> buildMockOffencesWithoutStatementOfFacts() {
        return asList(
                ...
                offence().withStatementOfFacts(STATEMENT_OF_FACTS).build(),
