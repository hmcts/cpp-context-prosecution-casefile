...
public class CroNumberValidationRuleTest {

    private static final String TEST_INVALID_CRO_NUMBER = "1234";
    private static final String TEST_VALID_CRO_NUMBER_ONE = "123456/20L";
    private static final String TEST_VALID_CRO_NUMBER_TWO = "SF20/123456M";
    ...
    public void shouldReturnEmptyListWhenCRONumberNotPresent() {
        ...
        final Optional<Problem> optionalProblem = new CroNumberValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
    ...
    @Test
    public void shouldReturnEmptyErrorListWhenValidCRONumberFormatOne() {
        when(defendantWithReferenceData.getDefendant()).thenReturn(getDefendant(TEST_VALID_CRO_NUMBER_ONE));

        final Optional<Problem> optionalProblemOne = new CroNumberValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
                .problems().stream().findFirst();
    ...
    @Test
    public void shouldReturnEmptyErrorListWhenValidCRONumberFormatTwo() {
        when(defendantWithReferenceData.getDefendant()).thenReturn(getDefendant(TEST_VALID_CRO_NUMBER_TWO));

        final Optional<Problem> optionalProblemTwo = new CroNumberValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
                .problems().stream().findFirst();
