...
public class ArrestDateValidationAndEnricherRuleTest {
    ...
    public void shouldReturnEmptyListWhenNoOffences() {
        ...
        final Optional<Problem> optionalProblem = new ArrestDateValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
                .problems().stream().findFirst();
        assertThat(optionalProblem.isPresent(), is(false));
    }

    @Test
    public void shouldReturnEmptyListWhenArrestDateInThePast() {
    ...
    @Test
    public void shouldReturnViolationWhenArrestDateIsNullWhenInitiationCodeIsC() {
        ...
        assertThat(optionalProblem.get().getCode(), is(ARREST_DATE_IN_FUTURE.name()));
        assertThat(optionalProblem.get().getValues().get(0).getKey(), is(OFFENCE_ARREST_DATE.getValue()));
        assertThat(optionalProblem.get().getValues().get(0).getValue(), is("Arrest date not provided"));
    }

    @Test
    public void shouldReturnViolationWhenArrestDateInTheFuture() {
        final LocalDate arrestDate = now().plusMonths(5);
        when(defendantWithReferenceData.getDefendant().getOffences()).thenReturn(getMockOffences(arrestDate));
        final Optional<Problem> optionalProblem = new ArrestDateValidationRule().validate(defendantWithReferenceData, referenceDataQueryService)
                .problems().stream().findFirst();
        assertThat(optionalProblem.isPresent(), is(true));
        assertThat(optionalProblem.get().getCode(), is(ARREST_DATE_IN_FUTURE.name()));
        assertThat(optionalProblem.get().getValues().get(0).getKey(), is(OFFENCE_ARREST_DATE.getValue()));
        assertThat(optionalProblem.get().getValues().get(0).getValue(), is(arrestDate.toString()));
    }

    private List<Offence> getMockOffences(final LocalDate arrestDate) {
