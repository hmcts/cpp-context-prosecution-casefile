...
public class CorporateDefendantSecondaryEmailAddressValidationRule implements ValidationRule<DefendantWithReferenceData, ReferenceDataQueryService> {

    private static final Pattern EMAIL_REGEX = Pattern.compile(EMAIL.getValue());

    @Override
    public ValidationResult validate(final DefendantWithReferenceData defendantWithReferenceData, final ReferenceDataQueryService referenceDataQueryService) {
        if (defendantWithReferenceData.getDefendant().getEmailAddress2() == null) {
            return VALID;
        }

        final String secondaryEmailAddress = defendantWithReferenceData.getDefendant().getEmailAddress2();
        if (secondaryEmailAddress == null) {
            return VALID;
        }
        return EMAIL_REGEX.matcher(secondaryEmailAddress).matches() ?
                VALID :
                newValidationResult(of(newProblem(DEFENDANT_SECONDARY_EMAIL_ADDRESS_INVALID, new ProblemValue(null, CORPORATE_DEFENDANT_SECONDARY_EMAIL_ADDRESS.getValue(), secondaryEmailAddress))));
    }
}
