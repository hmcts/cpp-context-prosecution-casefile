...
public class DateOfHearingValidationAndEnricherRule implements ValidationRule<DefendantWithReferenceData, ReferenceDataQueryService> {
    ...
    public ValidationResult validate(final DefendantWithReferenceData defendantWithReferenceData, final ReferenceDataQueryService referenceDataQueryService) {
        ...
        final boolean isBeforeOffenceStartDate = defendantWithReferenceData.getDefendant().getOffences().stream()
                .anyMatch(offence -> dateOfHearingDateFormat.isBefore(offence.getOffenceCommittedDate()));

        if (isBeforeOffenceStartDate) {
            return newValidationResult(of(newProblem(DATE_OF_HEARING_EARLIER_THAN_OFFENCE_COMMITTED_DATE, new ProblemValue(null, FieldName.DEFENDANT_DATE_OF_HEARING.getValue(), dateOfHearing))));
        } else {
            return VALID;
        }
    }
