...
public class SameOffenceCodeValidationRuleTest {

    @Mock
    private ReferenceDataQueryService referenceDataQueryService;

    @InjectMocks
    private SameOffenceCodeValidationRule sameOffenceCodeValidationRule;
    ...
    public void shouldNotReturnAnyValidationErrorsWhenSameOffenceCodeForAllTheCases() {
        ...
        final ValidationResult problem = sameOffenceCodeValidationRule.validate(groupProsecutionList, referenceDataQueryService);
    ...
    @Test
    public void shouldReturnValidationErrorWhenOffenceCodesAreDifferentAcrossProsecutionCases() {
        ...
        final ValidationResult result = sameOffenceCodeValidationRule.validate(groupProsecutionList, referenceDataQueryService);

        Problem problem = result.problems().stream().findFirst().orElse(null);

        assertThat(problem.getCode(), is("DIFFERENT_TYPE_CIVIL_OFFENCE_CODES_ARE_PRESENT"));
        assertThat(problem.getValues().size(), is(1));
        assertThat(problem.getValues().get(0).getKey(), is("more.than.one.different.offence.code"));
