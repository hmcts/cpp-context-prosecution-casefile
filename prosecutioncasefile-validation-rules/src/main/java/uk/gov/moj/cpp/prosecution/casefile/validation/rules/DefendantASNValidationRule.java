...
public class DefendantASNValidationRule extends AbstractValidationRule implements SubmitApplicationValidationRule {

    public DefendantASNValidationRule() {
        super(ValidationError.DEFENDANT_ASN_NOT_FOUND);
    }
    ...
    public Optional<ValidationError> validate(final SubmitApplication submitApplication, final AdditionalInformation additionalInformation) {
        ...
        if (nonNull(respondents)) {
            ...
            final boolean isDefendantUnMatched = respondents.stream().filter(Objects::nonNull)
                    .anyMatch(respondent -> !toBoolean(respondent.getIsDefendantMatched()) && !isNullOrEmpty(respondent.getAsn()));
