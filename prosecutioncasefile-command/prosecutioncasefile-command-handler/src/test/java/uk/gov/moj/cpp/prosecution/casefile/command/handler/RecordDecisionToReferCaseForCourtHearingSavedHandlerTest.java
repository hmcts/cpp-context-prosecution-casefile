...
public class RecordDecisionToReferCaseForCourtHearingSavedHandlerTest {
    ...
    @InjectMocks
    private RecordDecisionToReferCaseForCourtHearingSavedHandler recordDecisionToReferCaseForCourtHearingSavedHandler;

    private ProsecutionCaseFile aggregate = new ProsecutionCaseFile();

    @Test
    public void shouldHaveAHandlerForAcceptCourtReferralCommand() {
        assertThat(recordDecisionToReferCaseForCourtHearingSavedHandler, isHandler(COMMAND_HANDLER)
                .with(method("handleRecordDecisionToReferCaseForCaseHearing")
                        .thatHandles("prosecutioncasefile.command.record-decision-to-refer-case-for-court-hearing-saved")
                ));
    }
    ...
    public void shouldHandleTheAcceptCourtReferralWhenTheCaseIsReferredToCourt() throws EventStreamException {
        // given
        final RecordDecisionToReferCaseForCourtHearingSaved recordDecisionToReferCaseForCourtHearingSaved = RecordDecisionToReferCaseForCourtHearingSaved
                .recordDecisionToReferCaseForCourtHearingSaved()
                .withCaseId(CASE_ID)
                .withReferralReasonId(REFERRAL_REASON_ID)
                .build();

        final Envelope<RecordDecisionToReferCaseForCourtHearingSaved> envelope =
                envelopeFrom(metadataFor("prosecutioncasefile.command.record-decision-to-refer-case-for-court-hearing-saved"), recordDecisionToReferCaseForCourtHearingSaved);
        ...
        recordDecisionToReferCaseForCourtHearingSavedHandler.handleRecordDecisionToReferCaseForCaseHearing(envelope);
