...
public class SelfDefinedInformationRepositoryTest {
    ...
    @Test
    void shouldRemoveSelfDefinedInformation() {
        saveDefendantWithSelfDefinedInformation("British", Gender.MALE);

        final SelfDefinedInformationDetails found = selfDefinedInformationRepository.findBy(UUID.fromString(DEFENDANT_ID));
        assertThat(found, notNullValue());

        selfDefinedInformationRepository.remove(found);
        hibernateTestEntityManagerProvider.getEntityManager().clear();

        assertThat(selfDefinedInformationRepository.findBy(UUID.fromString(DEFENDANT_ID)), is(nullValue()));
    }
